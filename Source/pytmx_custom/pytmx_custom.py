...
class TiledTileLayer(TiledElement):
    ...
    def iter_data_range(self, startx, starty, endx, endy):
        """ Iterate over layer data
        ...
        for row in self.data[starty:endy]:
            for gid in row[startx:endx]:
                yield x, y, gid
                x += 1
            y += 1
            x = startx
    ...
    def tiles_range(self, startx, starty, endx, endy):
        """ Iterate over tile images of this layer
        ...
        """
        images = self.parent.images
        for x, y, gid in [i for i in self.iter_data_range(startx, starty, endx, endy) if i[2]]:
            yield x, y, images[gid]
